...
# ─────────────────────────────────────────────────────────────────────
# База данных с корректным fallback на SQLite при отсутствии psycopg2
# ─────────────────────────────────────────────────────────────────────
...
def _get_conn():
    ...
    try:
        ...
        # Автоматический откат на SQLite (без 500-ок)
...
def _upsert_rows(rows: List[Dict[str,Any]]) -> int:
    ...
    with _get_conn() as c:
        ...
        if _driver_name()=="sqlite":
            ...
            """
            for ch in _chunked(rows): cur.executemany(sql, ch); total += cur.rowcount or 0
        else:
            sql = """
            INSERT INTO bridge_sales
              (order_id,line_index,order_code,date_utc_ms,state,sku,title,qty,unit_price,total_price)
...
def _extract_entry(entry, inc_index) -> Optional[Dict[str,Any]]:
    ...
    if not title and titles: title = next(iter(titles.values()), "")
...
async def _fetch_entries_fallback(order_id: str) -> List[Dict[str,Any]]:
    if not (KASPI_FALLBACK_ENABLED and KASPI_TOKEN): return []
...
# ─────────────────────────────────────────────────────────────────────
# НОВОЕ: список строк с НОМЕРОМ ЗАКАЗА по SKU и периоду
# ─────────────────────────────────────────────────────────────────────
@router.get("/bridge/list")
def bridge_list(
    sku: str = Query(..., description="Искомый SKU (offer.code)"),
    ...
    # нормализуем дату и возвращаем «номер заказа» (order_code)
    items = []
    for r in rows:
