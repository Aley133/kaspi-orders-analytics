...
_engine = create_engine(DATABASE_URL, pool_pre_ping=True, future=True) if _USE_PG else None

def _resolve_db_path() -> str:
    p = os.getenv("DB_PATH", "/data/kaspi-orders.sqlite3")
...
def _rows(rs): return [dict(r._mapping) for r in rs] if _USE_PG else [dict(r) for r in rs]
...
def _parse_date_local(d: str, tz: str) -> datetime:
    z = _tzinfo(tz); y,m,dd = map(int, d.split("-"))
...
# ---------------- Schema (совместимо с FIFO) ----------------
def _ensure_schema():
    with _db() as c:
        if _USE_PG:
            # заказы / позиции (для отладки)
            ...
            # ГЛАВНОЕ: таблица продаж, из которой читает profit_fifo
...
def _norm_state(s: str) -> str: return (s or "").strip().upper()
    ...
    async with httpx.AsyncClient(base_url=KASPI_BASE_URL, timeout=30.0) as cli:
        ...
        try:
            ...
            for e in data:
                ...
                if entry_id and sku:
                    out.append({"entry_id": entry_id, "sku": sku, "qty": qty, "unit_price": price})
            if out: return out
...
# ---------------- Upsert ----------------
def _upsert_order_with_items(o: OrderIn) -> Tuple[int,int,int]:
    """
    Возвращает: (inserted_orders, inserted_order_items, upserted_sales_rows)
    """
    _ensure_schema()
    ins_o = ins_i = ins_s = 0
    with _db() as c:
        ...
        for it in o.items:
            if _USE_PG:
                c.execute(_q("""
                    INSERT INTO order_items(entry_id,order_id,sku,qty,unit_price,commission_pct)
                    VALUES(:eid,:oid,:sku,:qty,:p,:comm)
                """), {"eid": it.entry_id, "oid": o.id, "sku": it.sku.strip(),
                ...
                c.execute("""
                    INSERT INTO order_items(entry_id,order_id,sku,qty,unit_price,commission_pct)
                    VALUES(?,?,?,?,?,?)
                """, (it.entry_id, o.id, it.sku.strip(), int(it.qty), float(it.unit_price),
                      float(it.commission_pct) if it.commission_pct is not None else None))
            ins_i += 1

        # SALES (главная таблица для FIFO)
        for it in o.items:
            if _USE_PG:
                c.execute(_q("""
                    INSERT INTO sales(entry_id,order_id,date,sku,qty,unit_price,commission_pct)
                    VALUES(:eid,:oid,:date,:sku,:qty,:p,:comm)
                ...
                """), {"eid": it.entry_id, "oid": o.id, "date": o.date, "sku": it.sku.strip(),
                       "qty": int(it.qty), "p": float(it.unit_price),
                       "comm": float(it.commission_pct) if it.commission_pct is not None else None})
            else:
                c.execute("""
                    ...
                    ON CONFLICT(entry_id) DO UPDATE SET
                      ...
                      unit_price=excluded.unit_price,
                      commission_pct=excluded.commission_pct
                """, (it.entry_id, o.id, o.date, it.sku.strip(), int(it.qty),
                      float(it.unit_price),
                      float(it.commission_pct) if it.commission_pct is not None else None))
            ins_s += 1

    return ins_o, ins_i, ins_s
...
@router.get("/bridge/counts")
async def bridge_counts(_auth: bool = Depends(require_api_key)):
    _ensure_schema()
    with _db() as c:
        if _USE_PG:
            o = c.execute(_q("SELECT COUNT(*) n FROM orders")).scalar() or 0
            i = c.execute(_q("SELECT COUNT(*) n FROM order_items")).scalar() or 0
            s = c.execute(_q("SELECT COUNT(*) n FROM sales")).scalar() or 0
        else:
    ...
    return {"orders": o, "order_items": i, "sales": s}

@router.get("/bridge/sample")
async def bridge_sample(limit: int = 5, _auth: bool = Depends(require_api_key)):
    with _db() as c:
        if _USE_PG:
            rows = _rows(c.execute(_q("SELECT * FROM sales ORDER BY date DESC LIMIT :n"), {"n": limit}))
        else:
            rows = _rows(c.execute("SELECT * FROM sales ORDER BY date DESC LIMIT ?", (limit,)))
    return {"rows": rows}
...
async def profit_sync(
    ...
    if not orders:
        return {"status": "ok", "synced_orders": 0, "items_inserted": 0, "sales_upserted": 0}

    total_o = total_i = total_s = 0
    for od in orders:
        oid = str(od["id"])
        items_raw = await _fetch_entries_httpx(oid)
        if not items_raw:
            continue
        items: List[OrderItemIn] = [
            OrderItemIn(entry_id=i["entry_id"], sku=i["sku"],
                        qty=int(i["qty"]), unit_price=float(i["unit_price"]))
            for i in items_raw
        ]
        o = OrderIn(id=oid, date=od["date"], customer=od.get("customer"), items=items)
        io, ii, isales = _upsert_order_with_items(o)
        total_o += io; total_i += ii; total_s += isales

    return {"status": "ok", "synced_orders": total_o, "items_inserted": total_i, "sales_upserted": total_s}
